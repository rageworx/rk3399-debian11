...
if [[ "$1" == "px30" || "$1" == "rk3326" || "$1" == "rk3358" ]]; then
	...
	glmark2-es2 --visual-config='a=0:buf=24:stencil=8' --annotate
	...
	taskset -c 4-5 glmark2-es2 --visual-config='a=0:buf=24' --annotate
	...
	taskset -c 4-8 glmark2-es2 --visual-config='a=0:buf=24' --annotate
...
echo "run glmark2 x11 with window......"
